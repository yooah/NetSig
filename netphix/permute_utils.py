...
def read_permute_file(permutefile):
    ...
    TotCosts = []
    for l in lines[2:]:
        TotCosts.append(float(l.split()[cost_idx]))
    return TotCosts
...
def bipartite_double_edge_swap(B, genes, samples, nswap=1, max_tries=1e75):
    ...
    gkeys, gdegrees = zip(*dict(B.degree(genes)).items())  # keys, degree for genes
    gcdf = nx.utils.cumulative_distribution(gdegrees)  # cdf of degree for genes

    pkeys, pdegrees = zip(*dict(B.degree(samples)).items())  # keys, degree for samples
